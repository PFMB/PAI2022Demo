...
class BayesianLayer(nn.Module):
    def __init__(self, base_layer=nn.Linear, prior=Laplace(0, 5.0), *args, **kwargs):
        super().__init__()
...
class RegressionNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.w1 = base_layer(in_features=in_dim, out_features=128)
        self.w2 = base_layer(in_features=128, out_features=64)
        self.head = base_layer(in_features=64, out_features=out_dim)
        self.scale_head = base_layer(in_features=64, out_features=out_dim)
        if base_layer == BayesianLayer:
            self.w1.prior = prior
            self.w2.prior = prior
            self.head.prior = prior
            self.scale_head.prior = prior
